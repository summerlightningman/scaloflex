...
trait Reducer extends DataTypes {
  ...
  class ReduceExecutor(reduce: Reduce, outputPath: String) extends Actor with ActorLogging {
    ...
    //      case message: Exception => log.info(s"Shuffle has received a next error: ${message.printStackTrace()}")
    //      case message: String => log.info(s"Shuffle has received a non-correct text: $message")
